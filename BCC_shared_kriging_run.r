...
if (!file.exists(glue("{project_dir}/output/{dateStamp}-{note}-data"))) {
    dir.create(glue("{project_dir}/output/{dateStamp}-{note}-data"))
}
setwd(glue("{project_dir}/output/{dateStamp}-{note}-data"))
...
for (yy in y_vars) {
    for (zz in z_vars) {
        if (yy != zz) {
            ...
            export_dir <- glue("{project_dir}/output/kriging_runs/{filename}")
            choices <- c(xx, yy, zz)
