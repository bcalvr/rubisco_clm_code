...
for wt_org in clade:
    ...
    def specific_mutation_kriging_output(innie,date,note,X,Y,Z,filterN=0,filter_UQ_LQ_NL_NU=0,norm=False,save=False,scatter=True,scatterSave=False,maha=False,yzscatter=True,yzscatterSave=False):
        ...
        if save or yzscatterSave or scatterSave:
            data_directory = f'{home_dir}/results/{export_name}'
        ...
        if scatter:
            ...
            cb = fig.colorbar(plt.cm.ScalarMappable(cmap=cm.get_cmap('plasma'), norm=mpl.colors.Normalize(vmin=0, vmax=1)), ax=ax)
    ...
    for yy in vars:
        for zz in vars:
            if yy!=zz:
                ...
                specific_mutation_kriging_output(by_mutationDF_normed, dateStamp,runNote,f'{wtName}WTLoc',yy,zz,scatterSave=True,yzscatter=False,scatter=False,yzscatterSave=False,save=False)
    ...
    if not os.path.exists(f'{home_dir}/results'):
        os.mkdir(f'{home_dir}/results')
